...
bindPress(document.getElementById('mShootL'), () => { if (P1) fireShotgun(P1); });
bindPress(document.getElementById('mShootR'), () => { if (P2 && !botEnabled) fireShotgun(P2); });
document.getElementById('mView').addEventListener('click', () => toggleView());
...
function animate() {
  ...
  const uiActive = polyActive
    ...
    : (!started || matchOver);
  gameCursor.update(dt, uiActive && !isMobile); // no reticle on touch devices
